...
TEST(CSVOutputFormatterTest, SingleVectorBlankToStringEscaped) {
  ...
  std::string expected{"\r\n"};
  EXPECT_EQ(expected, csvio::util::CSVOutputFormatter<std::vector>::delim_join_escaped_fmt(to_join, ','));
}
...
TEST(CSVOutputFormatterTest, SingleVectorBlankToStringUnescaped) {
  std::vector<std::string> to_join{""};
  std::string expected{"\r\n"};
  EXPECT_EQ(expected, csvio::util::CSVOutputFormatter<std::vector>::delim_join_unescaped_fmt(to_join, ','));
}
...
TEST(CSVOutputFormatterTest, SingleListBlankToStringEscaped) {
  ...
  std::string expected{"\r\n"};
  EXPECT_EQ(expected, csvio::util::CSVOutputFormatter<std::list>::delim_join_escaped_fmt(to_join, ','));
}
...
TEST(CSVOutputFormatterTest, SingleListBlankToStringUnescaped) {
  std::list<std::string> to_join{""};
  std::string expected{"\r\n"};
  EXPECT_EQ(expected, csvio::util::CSVOutputFormatter<std::list>::delim_join_unescaped_fmt(to_join, ','));
}
